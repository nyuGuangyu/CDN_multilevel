...
dir = '/scratch/gl1257/CDN_multilevel/download_cost/optimal_results/'
...
class level3_graph(object): # feed popularity
    ...
    """
    def __init__(self, popularities, cache_catalog_percent, root,
                 level_factor = 1, linkcost_same = 1.,
                 d_b = 5., d_h_mid=5.5, d_h_up = 11., d_m_mid = 30.5, d_m_up = 61,
                 BOTTOM_LEVEL_PREFIX = 20, MID_LEVEL_PREFIX = 18, UP_LEVEL_PREFIX = 16,
                 downloadcost_mid = 2.,downloadcost_up = 1.,
        ...
        denom = 2*(MID_LEVEL_PREFIX - UP_LEVEL_PREFIX) + level_factor
        print 'denom:',denom
        self.level2_Cm = int(len(self.popularities.content_freq)*cache_catalog_percent*1./denom)
        self.level1_Cm = level_factor * self.level2_Cm # m caches in total, up level size=level_factor*mid level
        self.d_b = d_b
        ...
        self.downloadcost_mid = downloadcost_mid
        self.downloadcost_up = downloadcost_up

        self.prev_global_placement = prev_global_placement
        self.prev_mf_placement = prev_mf_placement
        self.prev_MFLRU_initial_placement = prev_MFLRU_initial_placement


        self.global_placement = defaultdict(list)
        self.global_routing = defaultdict(lambda:defaultdict(dict)) # 3-d dict to store p[i][j][m]
        self.glb_dc_mid_times = 0
        self.glb_dc_up_times = 0

        self.mf_placement = defaultdict(list)
        self.mf_routing = defaultdict(lambda:defaultdict(dict))  # 3-d dict to store p[i][j][m]
        self.mf_dc_mid_times = 0
        self.mf_dc_up_times = 0

        self.MFLRU_initial_placement = defaultdict(list)
        self.LRU_initial_placement = defaultdict(list)
        self.mflru_dc_mid_times = 0
        self.mflru_dc_up_times = 0
    ...
    def update_static_placement_routing(self):
        ...
        cnt = len(contents)
        prev_glb_ind = {}
        if self.prev_global_placement:
            for cache,its in self.prev_global_placement.items():
                cids = set()
                for it in its:
                    if it in content_cid:
                        cids.add(it)
                    else:
                        content_cid[it] = cnt
                        cids.add(cnt)
                        cnt += 1
                prev_glb_ind[cache_mid[cache]] = cids
        else:
            for mid in mid_cache:
                prev_glb_ind[mid] = set()

        prev_mf_ind = {}
        if self.prev_mf_placement:
            for cache, its in self.prev_mf_placement.items():
                cids = set()
                for it in its:
                    if it in content_cid:
                        cids.add(it)
                        ...
                        cids.add(cnt)
                        cnt += 1
                prev_mf_ind[cache_mid[cache]] = cids
        else:
            for mid in mid_cache:
                prev_mf_ind[mid] = set()

        prev_mflru_ind = {}
        if self.prev_MFLRU_initial_placement:
            for cache, its in self.prev_MFLRU_initial_placement.items():
                cids = set()
                for it in its:
                    if it in content_cid:
                        cids.add(it)
                        ...
                        cids.add(cnt)
                        cnt += 1
                prev_mflru_ind[cache_mid[cache]] = cids
        else:
            for mid in mid_cache:
                prev_mflru_ind[mid] = set()

        # greedy appox. without perf guarantees
        def greedy_placement(arrival_rate,q_ij,glb=True,mf=False):
            q = defaultdict(dict)
            lamda_i = {}
                        ...
                        for cid in S[mid]:
                            ...
                            for sid in range(len(_subgroups)):
                                if len(cache_ids[mid]) == self.MID_LEVEL_PREFIX \
                                    ...
                                    if glb and cid in prev_glb_ind[mid]:
                                        if _d[sid][cid] >= self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - self.d_h_mid)
                                    elif glb and cid not in prev_glb_ind[mid]:
                                        if _d[sid][cid] >= self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - self.d_h_mid - self.downloadcost_mid)
                                    elif mf and cid in prev_mf_ind[mid]:
                                        if _d[sid][cid] >= self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - self.d_h_mid)
                                    elif mf and cid not in prev_mf_ind[mid]:
                                        if _d[sid][cid] >= self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - self.d_h_mid - self.downloadcost_mid)
                                    ...
                                    if glb and cid in prev_glb_ind[mid]:
                                        ...
                                        if _d[sid][cid] >= self.d_h_up - self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - (self.d_h_up - self.d_h_mid)-self.downloadcost_up)
                                    elif mf and cid in prev_mf_ind[mid]:
                                        if _d[sid][cid] >= self.d_h_up - self.d_h_mid:
                                            sum_gain += _lamda_i[sid] * _q[sid][cid] * (
                                                _d[sid][cid] - (self.d_h_up - self.d_h_mid))
        ...
        if not (os.path.exists(filename1) and os.path.exists(filename2)):
            self.global_placement,self.global_routing = greedy_placement(arrival_rate,global_probs,glb=True,mf=False)
        ...
        filename2 = dir + str(len(contents)) + "_" + str(len(subgroups)) + '_' + str(self.cache_catalog_percent) \
                   + ' mf_routing.cp'
        if not (os.path.exists(filename1) and os.path.exists(filename2)):
            self.mf_placement,self.mf_routing = greedy_placement(arrival_rate,MF_probs,glb=False,mf=True)
            with open(filename1,'wb') as outfile1, open(filename2,'wb') as outfile2:
        ...
        # calculate count of download times.
        for cache in caches:
            if len(cache) == self.MID_LEVEL_PREFIX:
                for c in self.global_placement:
                    if c not in self.prev_global_placement:
                        self.glb_dc_mid_times += 1
                for c in self.mf_placement:
                    if c not in self.prev_mf_placement:
                        self.mf_dc_mid_times += 1
            else:
                for c in self.global_placement:
                    if c not in self.prev_global_placement:
                        self.glb_dc_up_times += 1
                for c in self.mf_placement:
                    if c not in self.prev_mf_placement:
                        self.mf_dc_up_times += 1
        print 'glb download times:mid:',self.glb_dc_mid_times
        print 'glb download times:up:', self.glb_dc_up_times
        print 'mf download times:mid:', self.mf_dc_mid_times
        print 'mf download times:up:',self.mf_dc_up_times
    ...
    def get_initial_MFLRU_placement(self,train,propotion = 0.6):
        ...
        for cache in caches:
            if len(cache) == self.MID_LEVEL_PREFIX:
                for c in self.MFLRU_initial_placement:
                    if c not in self.prev_MFLRU_initial_placement:
                        self.mflru_dc_mid_times += 1
            else:
                for c in self.MFLRU_initial_placement:
                    if c not in self.prev_MFLRU_initial_placement:
                        self.mflru_dc_up_times += 1
        print 'mflru initial download times:mid:', self.mflru_dc_mid_times
        print 'mflru initial download times:up:', self.mflru_dc_up_times

...
class policy_evaluator(object):
    ...
    def evaluate(self, pLRU = False, static_caching = True, MFLRU = False):
        ...
        if static_caching:
            ...
            cnt_dc_mid = self.level3graph.glb_dc_mid_times
            cnt_dc_up = self.level3graph.glb_dc_up_times
            self.print_perf()
            self.cnt_list.append([self.cnt_d_b, self.cnt_d_h_mid, self.cnt_d_h_up, cnt_dc_mid, cnt_dc_up])
            ...
            cnt_dc_mid = self.level3graph.mf_dc_mid_times
            cnt_dc_up = self.level3graph.mf_dc_up_times
            self.print_perf()
        ...
        if MFLRU:
            ...
            cnt_dc_mid = self.level3graph.mflru_dc_mid_times
            cnt_dc_up = self.level3graph.mflru_dc_up_times

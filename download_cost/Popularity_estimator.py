...
dir = '/scratch/gl1257/CDN_multilevel/download_cost/pops/'
...
class Popularity_estimator(object):
    ...
    def __get_rating_matrix(self):
        ...
        print 'Removing contents accessed less than', COLD_ITEM_LIMIT, ' times...'
    ...
    def calculate_global_pop(self):
        filename = dir + str(len(self.content_freq))+'_'+str(train_length)+'_glb_pop.cp'
    ...
    def calculate_MF_pop(self):
        filename = dir + str(len(self.content_freq)) + "_" + str(len(self.subgroup_item_freq))+'_'+str(train_length) \
                   + '_mf_pop.cp'
        if not os.path.exists(filename):
